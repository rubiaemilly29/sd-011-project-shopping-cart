...
function createCartItemElement({ id: sku, title: name, price: salePrice }, contador) { // redefino as chaves aos valores recebidos da api
...
// fonte de pesquisa --> https://github.com/tryber/sd-011-project-shopping-cart/pull/56/files
const addproductToCard = () => {
  const getOl = document.querySelector(newOlClass); // chamando o pai
...
const getLocalStorage = () => {
  const getOl = document.querySelector(newOlClass); // para limpar o ol
  for (let index = 0; index < localStorage.length; index += 1) {
    const getItemLocal = localStorage.getItem(`item ${index}`); // guarda, apos pegar itens e os armazena no localStorage
...
const getProduct = () => {
  fetch('https://api.mercadolibre.com/sites/MLB/search?q={computador}')
    ...
    const toCreate = createProductItemElement({ sku: id, name: title, image: thumbnail }); // redefino as chaves aos valores recebidos da api
...
const deleteButton = () => {
  ...
  getButton.addEventListener('click', () => {
    document.querySelector(newOlClass).innerHTML = '';
  localStorage.clear();
  });
};
