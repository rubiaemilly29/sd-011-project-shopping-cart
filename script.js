/** Consultei meu proprio repositório, onde ja havia desenvolvido este projeto.
Link do repositório https://github.com/tryber/sd-09-project-shopping-cart/pull/129
*/
...
function sumValue() {
  let sum = 0;
  const totalValue = document.querySelector(totalPriceClass);
...
async function fetchAPIML(QUERY) { // funcao assincrona de requisicao a API e listagem de produtos encontrados
  ...
  const { results } = object; // 'results' recebe os valores da chave "results"
  // do JSON retornado pela requisicao a API na forma de array de objetos
  ...
  removeLoadingAlert(); // remove alerta de carregamento
  sumValue();
}
...
async function fetchID(sku) { // requisicao feita a partir do valor da chave id do produto
  ...
  await fetch(`https://api.mercadolibre.com/items/${sku}`)
    ...
    .then((data) => {
      ...
      const list = document.querySelector(cartItemsClass);
